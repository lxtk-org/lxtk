/*******************************************************************************
 * Copyright (c) 2020, 2021 Julian Honnen and others.
 *
 ...
 *     Vladimir Piskarev - adaptation
 *          (adapted from org.eclipse.jdt.core.compiler.SubwordMatcher)
 *******************************************************************************/
...
class SubwordMatcher {
	...
	private int indexOfWordStart(int nameStart, char patternChar) {
		...
		for (int iName = nameStart; iName < this.name.length; iName++) {
			...
			if (!Character.isUnicodeIdentifierPart(nameChar)) {
				return -1;
			}
		}
